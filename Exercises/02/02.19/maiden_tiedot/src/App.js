...
const App = () => {
  ...
  const showCountry = (country) => {
    ...
    setFinalCountry(countries.find(c => c.name.common.toLowerCase().includes(country.toLowerCase())))
  }
  ...
  return (
    <div>
      ...
      <pre>
        {selectedCountries.map(country => <Country key={country.name.common} country={country} showCountry={() => showCountry(country.name.common)} />)}
      </pre>
...
const FinalCountry = ({ country }) => {
  console.log("FinalCountry", country)
  if (!country) {
    return null
  }
  return (
